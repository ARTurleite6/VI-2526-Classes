...
RGB PathTracingShader::IndirectIllumination(const Ray& ray, const Scene& scene, const Intersection& intersection, const Material& material, int depth, bool allow_emissive [[maybe_unused]]) const
{
  ...
  // Practical exercise baseline:
  // choose the diffuse Lambertian lobe and the specular GGX lobe with equal
  // probability. This is simple and unbiased as long as the same probability
  // is also used in the MIS weights below.
  //
  // Improvement for students:
  // replace this fixed value with material.GetSpecularProbability(). That lets
  // rough diffuse materials sample Lambert more often, while metallic or glossy
  // materials sample GGX more often.
  const float microfacet_probability = 0.5f;

  // Stochastically select which BRDF lobe generates the next ray direction.
  // GGX is useful for specular/glossy reflection; Lambert is useful for
  // diffuse scattering.
  ...
  // This is the PDF of the lobe that actually generated wi_local.
  float pdf = (sample_microfacet ? microfacet_pdf : diffuse_pdf);
  if (pdf <= 0.f)    return RGB{0.0f};
    
  // MIS compares the probability of this same direction under both sampling
  // strategies. The lobe PDFs are multiplied by the probability of choosing
  // that lobe. For the baseline exercise those probabilities are both 0.5.
  //
  // See PBRT, equations 2.14 and 2.15:
